#!/usr/bin/env python3
"""Append the source-order Chapter 9–20 audit ledger entries.
...
ledger = [row for row in ledger if not 9 <= row["chapter"] <= 20]

for chapter_number in range(9, 21):
...
print(f"Wrote {len(ledger)} inventory mappings ({sum(r['chapter'] >= 9 for r in ledger)} for Chapters 9–20).")
